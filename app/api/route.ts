...
function buildMockExtractedFromText(inputText: string) {
  // Deterministic-ish mock for MVP wiring tests.
  // Goal: exercise derive.ts + inference.ts without calling any upstream provider.
  const t = String(inputText || "");
  const len = t.length;
  const words = t.trim() ? t.trim().split(/\s+/).length : 0;

  // Coarse proxies
  const units = Math.max(1, Math.min(18, Math.round(words / 35) + 3));
  const claims = Math.max(1, Math.min(16, Math.round(words / 55) + 4));
  const reasons = Math.max(1, Math.min(20, Math.round(claims * 1.3)));
  const evidence = Math.max(0, Math.min(18, Math.round(words / 80) + 2));

  const revisions = Math.max(0, Math.min(14, Math.round(Math.log2(len + 1)) - 5));
  const transitions = Math.max(0, Math.min(20, Math.round(units * 0.8)));
  const transition_ok = Math.max(0, Math.min(transitions, Math.round(transitions * 0.75)));

  const warrants = Math.max(0, Math.min(10, Math.round(claims * 0.6)));
  const counterpoints = Math.max(0, Math.min(8, Math.round(claims * 0.25)));
  const refutations = Math.max(0, Math.min(8, Math.round(counterpoints * 0.5)));

  const intent_markers = Math.max(0, Math.min(12, Math.round(words / 140) + 1));
  const hedges = Math.max(0, Math.min(14, Math.round(words / 120) + 1));
  const loops = Math.max(0, Math.min(10, Math.round(revisions * 0.4)));
  const self_regulation_signals = Math.max(0, Math.min(10, Math.round(hedges * 0.5)));

  const structure_type =
    counterpoints >= 3 ? "networked" : units >= 10 ? "hierarchical" : "linear";

  const evidence_types = {
    data: Math.max(0, Math.round(evidence * 0.35)),
    authority: Math.max(0, Math.round(evidence * 0.25)),
    example: Math.max(0, evidence - (Math.max(0, Math.round(evidence * 0.35)) + Math.max(0, Math.round(evidence * 0.25))))
  };
  ...
  return {
    gpt_raw: {
      raw_features: {
        ...
        transitions,
        transition_ok,
        transition_types: ["cause_effect", "contrast", "generalization"].slice(0, Math.max(1, Math.min(3, Math.round(units/6)))),
        revisions,
        revision_depth_sum: Math.max(0, revisions * 2),
        belief_change: counterpoints >= 4,
        evidence_types,
        intent_markers,
        drift_segments: Math.max(0, Math.round(units * 0.15)),
        hedges,
        loops,
        self_regulation_signals,
        adjacency_links: Math.max(0, Math.round(units * 1.2)),
        cross_links: Math.max(0, Math.round(counterpoints * 1.4)),
      },
    },
  };
}
...
async function callOpenAIJson(args: { system: string; user: string }) {
  ...
  if (!apiKey) throw new Error("ENGINE_MISCONFIGURED");
  ...
  if (!resp.ok) throw new Error("ENGINE_UPSTREAM_FAILURE");

  const data = await resp.json();
  const content = data?.choices?.[0]?.message?.content ?? "";
  const parsed = safeJsonParse(content);
  if (!parsed) throw new Error("ENGINE_OUTPUT_PARSE_FAILURE");
  return parsed;
}

function sanitizeForClient(report: any) {
  const out = deepClone(report);

  // Neutralize extraction naming
  if (out?.gpt_raw != null) {
    out.extracted = out.gpt_raw;
    delete out.gpt_raw;
  }

  // Remove obvious upstream/SDK hints if present
  delete out.model;
  delete out.usage;
  delete out.openai;
  delete out.openai_usage;
...
export async function POST(req: Request) {
  try {
    ...
    if (!text) return NextResponse.json({ error: { code: "BAD_REQUEST", message: "Invalid input." } }, { status: 400 });
    ...
    // 2) Extraction (raw_features only)
    // - In MOCK mode, we synthesize extracted.raw_features from the input text.
    // - In REAL mode, this is where you'll call your upstream LLM to return raw_features JSON.
    const { system, user } = buildGptRawExtractorMessages({
      inputText: text,
      inputLanguage: input_language,
    });

    const useMock = process.env.NP_MOCK === "1" || !process.env.OPENAI_API_KEY;

    const extracted = useMock
      ? buildMockExtractedFromText(text)
      : await callOpenAIJson({ system, user });
    if (extracted?.gpt_raw) report.gpt_raw = { ...(report.gpt_raw || {}), ...(extracted.gpt_raw || {}) };
    ...
    return NextResponse.json(sanitizeForClient(final), { status: 200 });
  } catch (e: any) {
    // Do not leak upstream provider details or internal stack traces.
